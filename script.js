...
const choice_town_form = document.querySelector('.choice_town_form');
const cityExample = document.querySelector('.choice_example > strong');
const bookmarkBtn = document.querySelector('.bookmarkBtn input');
const choice_izhevsk = document.querySelector('.choice_izhevsk');
const field_cards = document.querySelector(".field_cards"); 
...
cities.forEach((citi) => {
    ...
    field_cards.insertAdjacentHTML('beforeend', card);

    if(choice_izhevsk) choice_izhevsk.remove()
});
...
function showWeatherData() {
    choice_town_form.addEventListener('keydown', (e) => {
        if(e.key === 'Enter') {
            e.preventDefault()
